...
def cgol_iter(arr: np.ndarray) -> np.ndarray:
    ...
    for x in range(X):
        for y in range(Y):
            ngh = arr[
                max(0, x - 1) : min(X, x + 2), max(0, y - 1) : min(Y, y + 2)
            ].sum()
            if arr[x, y]:
                ngh -= 1
                if ngh in (2, 3):
...
def cgol_iter2(arr: np.ndarray) -> np.ndarray:
    ...
    for x in range(x_min, x_max):
        for y in range(y_min, y_max):
            ...
            if arr[x, y]:
                if ngh in (2, 3):
                    arr_next[x, y] = 1  # Stay alive
            else:
                if ngh == 3:
                    arr_next[x, y] = 1  # Become alive
    return arr_next
...
def cgol_iter3(arr: np.ndarray) -> np.ndarray:
    ...
    return np.logical_or((ngh_arr == 3), np.logical_and(arr, (ngh_arr == 2))).astype(
        np.uint8
    )
...
def board_to_img2(
    arr: np.ndarray, px_size: int, a: tuple = (255, 255, 255), d: tuple = (0, 0, 0)
) -> Image:
    # print('converting to image...')
    X = arr.shape[0]
    Y = arr.shape[1]
    ...
    arr_rep = arr.repeat(px_size, 0).repeat(px_size, 1).astype(np.int16)
    # For each channel, set alive or dead value based on arr_rep
    for c in (0, 1, 2):
        arr_img[:, :, c] = arr_rep * (a[c] - d[c]) + d[c]
