...
def send_game_req(fout, board: np.ndarray, length, width, max_len):
    ...
    board_in = np.pad(
        board_in, (0, trace_data_width), "constant", constant_values=0
    )  # Pad with 0s
    wr_idx = 0
    while wr_idx < width**2:
        if wr_idx == 0:
            ...
            fout.write(
                f"0001____0_00000000000_{''.join([str(x) for x in data])}_{game_len_bin}\n"
            )
        else:
...
def send_game_check(fout, board: np.ndarray, width):
    ...
    board = board.squeeze()
    board = np.pad(
        board, (0, trace_data_width), "constant", constant_values=0
    )  # Pad with 0s
    wr_idx = 0
    while wr_idx < width**2:
...
def add_image_to_list(list, board, disp):
    ...
    list.append(
        cgol.board_to_img2(
            board, disp["pixel_size"], a=disp["alive_color"], d=disp["dead_color"]
        )
    )
...
def main():
    ...
    parser.add_argument(
        "-cfg", required=True, help="The input config file (in yaml format)"
    )
    parser.add_argument(
        "-out",
        required=False,
        default=default_trace_fout,
    ...
    with open(args.out, "w") as fout:
        ...
        fout.write("")
        fout.write(
            f'# This trace file was generated by "final_project_pre_sim.py" with the config file "{args.cfg}", do not directly modify!\n'
        )
        fout.write("\n")
        fout.write(
            f"# Board size: {width}x{width}, max game length: {cfg['board']['max_length']}\n"
        )
        ...
        for idx, game in enumerate(cfg["games"]):
            assert game["length"] <= max_len, (
                f"Max game length exceeded! (max: {max_len}, requested: {game['length']})"
            )
            ...
            if game["origin"] == "center":
                for ii, p in enumerate(alive_list):
                    alive_list[ii] = [p[0] + (width // 2), p[1] + (width // 2)]
            for pt in alive_list:
                board[pt[0]][pt[1]] = 1
            ...
            if game["checks"] == "last":
                send_game_req(
                    fout, board, game["length"], width, max_len
                )  # Send initial data
                ...
                cgol.save_gif(
                    images,
                    os.path.join(output_dir, expected_fout),
                    frame_dur=cfg["display"]["frame_dur_ms"],
                )
            ...
            # Generate the trace for this game: check every frame a long the way
                ...
                with tqdm(range(game["length"]), disable=False) as tq:
                    for i in tq:
                        ...
                        send_game_req(
                            fout, board, 1, width, max_len
                        )  # Send next frame data (length=1)
                        add_image_to_list(images, board, cfg["display"])
                        board = cgol.cgol_iter3(board)
                        send_game_check(fout, board, width)  # Send output check
                add_image_to_list(images, board, cfg["display"])  # Add last frame
    ...
    print(
        f"   Computed and saved a total of {cgol_iterations} CGoL frames of size {width}x{width} over {cgol_elapsed_time:.3f} seconds"
    )
    if cgol_elapsed_time == 0:
        cgol_elapsed_time = float("nan")  # Handle case where there were no 'last' games
    print(
        f"   Average performance {cgol_iterations / cgol_elapsed_time:.3f} frames/second"
    )
    print()
